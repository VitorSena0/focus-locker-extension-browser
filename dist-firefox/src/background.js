...
async function updateBadge() {
  ...
  if (status === SESSION_STATUS.RUNNING) {
    await ext.action.setBadgeBackgroundColor({ color: "#1144cc" });
    return;
  }

  if (status === SESSION_STATUS.PAUSED_MANUAL || status === SESSION_STATUS.PAUSED_IDLE) {
    await ext.action.setBadgeBackgroundColor({ color: "#cc7a00" });
    return;
  }

  if (status === SESSION_STATUS.COMPLETED_PENDING_CONFIRMATION) {
    await ext.action.setBadgeBackgroundColor({ color: "#006d32" });
    return;
  }

  if (status === SESSION_STATUS.UNLOCKED_UNTIL_MIDNIGHT) {
    await ext.action.setBadgeBackgroundColor({ color: "#2a9d8f" });
    return;
  }

  await ext.action.setBadgeBackgroundColor({ color: "#6c757d" });
}
